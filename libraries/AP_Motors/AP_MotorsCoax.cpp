...
void AP_MotorsCoax::output_to_motors()
{
	...
	switch (_spool_state) {
		...
		case SpoolState::GROUND_IDLE:
			...
			motor1.values[2]=output_to_pwm(_actuator[5]);
			motor2.values[2]=output_to_pwm(_actuator[6]);
			motor1.sendValues();
			//motor2.sendValues();
			break;
...
void AP_MotorsCoax::output_armed_stabilizing()
{
	...
	//angle values
	//arctan (roll / pitch)
	//TODO- problem here is the negatives
	if ((_actuator_out[1] >= 0) && (_actuator_out[0] >= 0)){
		motor1.values[0] = (atan2f(_actuator_out[0], _actuator_out[1])) * (180 / 3.14159265359);
	}
	else if ((_actuator_out[1] <= 0) && (_actuator_out[0] >= 0)){
		motor1.values[0] = 90 - ((atan2f(_actuator_out[1], _actuator_out[0])) * (180 / 3.14159265359));
	}
	else if ((_actuator_out[1] <= 0) && (_actuator_out[0] <= 0)){
		motor1.values[0] = 180 + ((atan2f(_actuator_out[0], _actuator_out[1])) * (180 / 3.14159265359));
	}
	else if ((_actuator_out[1] >= 0) && (_actuator_out[0] <= 0)){
		motor1.values[0] =360 + ((atan2f(_actuator_out[0], _actuator_out[1])) * (180 / 3.14159265359));
	}
	else{
		motor1.values[0] = 0;
	}

	motor2.values[0] = motor1.values[0];	//copy it over to second motor

	motor1.values[1] = (safe_sqrt((_actuator_out[1]*_actuator_out[1]) + (_actuator_out[0]*_actuator_out[0])));
	motor2.values[1] = motor1.values[1];
}
